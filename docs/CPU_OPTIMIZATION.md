...
### 3. `src/utils/CovenantCommands.ts`
**Purpose**: Console commands for debugging and monitoring

**Available Commands** (via `Game.cov`):
```javascript
Game.cov.profile(0.1)      // Show CPU profile report (min 0.1 CPU)
Game.cov.resetProfile()    // Reset all profiling data
Game.cov.cacheStats()      // Show cache statistics
Game.cov.clearCache()      // Clear all caches
Game.cov.cpuStatus()       // Show current CPU usage & budget
Game.cov.topCpu(10)        // Show top 10 CPU consumers
Game.cov.colony('W1N1')    // Show colony status for room
Game.cov.colonies()        // List all High Charities
Game.cov.help()            // Show command help
```

## Integrations

### Modified: `src/core/HighCharity.ts`
**Changes**:
- Added profiler imports and tick budget checks
- Wrapped all phase methods with profiling:
  - `build()` - Profiled as `HighCharity_{room}_build`
  - `init()` - Profiled as `HighCharity_{room}_init`
  - `run()` - Profiled as `HighCharity_{room}_run`
- Added per-system profiling for Temples and Arbiters
...
### Modified: `src/core/Covenant.ts`
**Changes**:
- Added CovenantCommands integration
- Initialize commands in constructor
- Commands accessible via Game.cov

### Modified: `src/main.ts`
**Changes**:
- Added profiler wrapping for all Covenant phases:
  - `Covenant_build`
  - `Covenant_init`
  - `Covenant_run`
  - `Covenant_endOfTick`
...
- Exposed console commands via `Game.cov`

### Modified: `src/elites/Elite.ts`
**Changes**:
...
Game.cov.profile(1.0); // Only show 1+ CPU operations
```
...
Edit threshold values in HighCharity.ts `run()`:
```typescript
...
- Use `Game.cov.resetProfile()` to start fresh
...
**Next Steps**: Monitor CPU usage with `Game.cov.cpuStatus()` and optimize high consumers identified by `Game.cov.topCpu(10)`.
