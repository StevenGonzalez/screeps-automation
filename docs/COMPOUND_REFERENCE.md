# Compound Boost Reference

Complete reference of all compounds, their production chains, and boost effects.

## Tier 4 - Catalyzed (Ultimate Boosts)

### 🗡️ XUH2O - Catalyzed Utrium Acid
**Production Chain:** U + H → UH → UH2O → XUH2O  
**Ingredients:** UH2O + X (catalyst)  
**Boost:** ATTACK +300%  
**Priority:** 100 (highest)  
**Uses:** Melee combat, dismantling structures

---

### 💚 XUHO2 - Catalyzed Utrium Alkalide
**Production Chain:** U + O → UO → UHO2 → XUHO2  
**Ingredients:** UHO2 + X (catalyst)  
**Boost:** HEAL +300%  
**Priority:** 90  
**Uses:** Healing, support squads

---

### 🏹 XKHO2 - Catalyzed Keanium Alkalide
**Production Chain:** K + O → KO → KHO2 → XKHO2  
**Ingredients:** KHO2 + X (catalyst)  
**Boost:** RANGED_ATTACK +300%  
**Priority:** 85  
**Uses:** Ranged combat, room clearing

---

### 🏗️ XLH2O - Catalyzed Lemergium Acid
**Production Chain:** L + H → LH → LH2O → XLH2O  
**Ingredients:** LH2O + X (catalyst)  
**Boost:** BUILD +100%  
**Priority:** 80  
**Uses:** Fast construction, remote bases

---

### 🔧 XLHO2 - Catalyzed Lemergium Alkalide
**Production Chain:** L + O → LO → LHO2 → XLHO2  
**Ingredients:** LHO2 + X (catalyst)  
**Boost:** REPAIR +100%  
**Priority:** 75  
**Uses:** Defense repair, wall maintenance

---

### ⛏️ XZH2O - Catalyzed Zynthium Acid
**Production Chain:** Z + H → ZH → ZH2O → XZH2O  
**Ingredients:** ZH2O + X (catalyst)  
**Boost:** DISMANTLE +300%  
**Priority:** 70  
**Uses:** Structure destruction, resource extraction

---

### 🏃 XZHO2 - Catalyzed Zynthium Alkalide
**Production Chain:** Z + O → ZO → ZHO2 → XZHO2  
**Ingredients:** ZHO2 + X (catalyst)  
**Boost:** MOVE -100% fatigue (2x speed)  
**Priority:** 65  
**Uses:** Fast movement, kiting, hauling

---

### ⚡ XGH2O - Catalyzed Ghodium Acid
**Production Chain:** Z + K → G → GH → GH2O → XGH2O  
**Ingredients:** GH2O + X (catalyst)  
**Boost:** UPGRADE_CONTROLLER +100%  
**Priority:** 60  
**Uses:** Rapid GCL progression

---

### 🛡️ XGHO2 - Catalyzed Ghodium Alkalide
**Production Chain:** Z + K → G → GO → GHO2 → XGHO2  
**Ingredients:** GHO2 + X (catalyst)  
**Boost:** TOUGH +300%  
**Priority:** 55  
**Uses:** Tanks, damage reduction

---

### 📦 XKH2O - Catalyzed Keanium Acid
**Production Chain:** K + H → KH → KH2O → XKH2O  
**Ingredients:** KH2O + X (catalyst)  
**Boost:** CARRY +100%  
**Priority:** 50  
**Uses:** Heavy hauling, logistics

---

## Tier 3 - Boosted (Strong Boosts)

### UH2O - Utrium Acid
**Ingredients:** UH + OH  
**Boost:** ATTACK +200%  
**Priority:** 45

### UHO2 - Utrium Alkalide
**Ingredients:** UO + OH  
**Boost:** HEAL +200%  
**Priority:** 44

### KH2O - Keanium Acid
**Ingredients:** KH + OH  
**Boost:** CARRY +50%  
**Priority:** 36

### KHO2 - Keanium Alkalide
**Ingredients:** KO + OH  
**Boost:** RANGED_ATTACK +200%  
**Priority:** 43

### LH2O - Lemergium Acid
**Ingredients:** LH + OH  
**Boost:** BUILD +50%  
**Priority:** 42

### LHO2 - Lemergium Alkalide
**Ingredients:** LO + OH  
**Boost:** REPAIR +50%  
**Priority:** 41

### ZH2O - Zynthium Acid
**Ingredients:** ZH + OH  
**Boost:** DISMANTLE +200%  
**Priority:** 40

### ZHO2 - Zynthium Alkalide
**Ingredients:** ZO + OH  
**Boost:** MOVE -50% fatigue  
**Priority:** 39

### GH2O - Ghodium Acid
**Ingredients:** GH + OH  
**Boost:** UPGRADE_CONTROLLER +50%  
**Priority:** 38

### GHO2 - Ghodium Alkalide
**Ingredients:** GO + OH  
**Boost:** TOUGH +200%  
**Priority:** 37

---

## Tier 2 - Advanced (Weak Boosts / Intermediates)

### UH - Utrium Hydride
**Ingredients:** U + H  
**Boost:** ATTACK +100%  
**Priority:** 20

### UO - Utrium Oxide
**Ingredients:** U + O  
**Boost:** HEAL +100%  
**Priority:** 19

### KH - Keanium Hydride
**Ingredients:** K + H  
**Boost:** CARRY +25%  
**Priority:** 18

### KO - Keanium Oxide
**Ingredients:** K + O  
**Boost:** RANGED_ATTACK +100%  
**Priority:** 17

### LH - Lemergium Hydride
**Ingredients:** L + H  
**Boost:** BUILD +25%  
**Priority:** 16

### LO - Lemergium Oxide
**Ingredients:** L + O  
**Boost:** REPAIR +25%  
**Priority:** 15

### ZH - Zynthium Hydride
**Ingredients:** Z + H  
**Boost:** DISMANTLE +100%  
**Priority:** 14

### ZO - Zynthium Oxide
**Ingredients:** Z + O  
**Boost:** MOVE -25% fatigue  
**Priority:** 13

### GH - Ghodium Hydride
**Ingredients:** G + H  
**Boost:** UPGRADE_CONTROLLER +25%  
**Priority:** 12

### GO - Ghodium Oxide
**Ingredients:** G + O  
**Boost:** TOUGH +100%  
**Priority:** 11

---

## Tier 1 - Base (No Boosts / Used as Ingredients)

### OH - Hydroxide
**Ingredients:** O + H  
**Boost:** None (used for Tier 3 production)  
**Priority:** 30 (high because needed for all Tier 3)  
**Uses:** Essential intermediate for all Tier 3 compounds

### G - Ghodium
**Ingredients:** Z + K  
**Boost:** None (used for GH/GO production)  
**Priority:** 10

### ZK - Zynthium Keanite
**Ingredients:** Z + K  
**Boost:** None (commodity production)  
**Priority:** 5

### UL - Utrium Lemergite
**Ingredients:** U + L  
**Boost:** None (commodity production)  
**Priority:** 4

---

## Base Minerals (No Production)

Must be obtained through mining or market:

- **H** (Hydrogen) - Source harvest or market
- **O** (Oxygen) - Source harvest or market
- **U** (Utrium) - Mineral harvest or market
- **K** (Keanium) - Mineral harvest or market
- **L** (Lemergium) - Mineral harvest or market
- **Z** (Zynthium) - Mineral harvest or market
- **X** (Catalyst) - Mineral harvest or market

---

## Boost Usage Examples

### Combat Squad (Attack + Heal)
```javascript
Game.kha.produce('XUH2O', 3000)  // Attack boost
Game.kha.produce('XUHO2', 3000)  // Heal boost
Game.kha.produce('XZHO2', 2000)  // Movement boost
```

### Builder Squad (Fast Construction)
```javascript
Game.kha.produce('XLH2O', 3000)  // Build boost
Game.kha.produce('XZHO2', 2000)  // Movement boost
```

### Upgrader Squad (Fast GCL)
```javascript
Game.kha.produce('XGH2O', 5000)  // Upgrade boost
```

### Dismantler Squad (Structure Destruction)
```javascript
Game.kha.produce('XZH2O', 3000)  // Dismantle boost
Game.kha.produce('XZHO2', 2000)  // Movement boost
```

### Defense Squad (Repair + Ranged)
```javascript
Game.kha.produce('XLHO2', 3000)  // Repair boost
Game.kha.produce('XKHO2', 3000)  // Ranged boost
```

---

## Production Time Estimates

Assuming 6 labs (2 input, 4 output), no cooldowns:

| Compound | Tier | Reactions | Time Estimate |
|----------|------|-----------|---------------|
| OH | 1 | 1 step | ~10 ticks/1000 units |
| UH | 2 | 1 step | ~10 ticks/1000 units |
| UH2O | 3 | 2 steps | ~20 ticks/1000 units |
| XUH2O | 4 | 3 steps | ~30 ticks/1000 units |

**Note:** Each lab has 10-tick cooldown, so actual production is ~4 units per tick per output lab.

With 4 output labs: ~16 units/tick = ~960 units/minute

To produce 3000 units of XUH2O (all tiers):
- OH: 3000 units ≈ 188 ticks ≈ 3 minutes
- UH: 3000 units ≈ 188 ticks ≈ 3 minutes  
- UH2O: 3000 units ≈ 188 ticks ≈ 3 minutes
- XUH2O: 3000 units ≈ 188 ticks ≈ 3 minutes

**Total:** ~750 ticks ≈ 12 minutes for full chain

---

## Boost Efficiency

| Boost Level | % Increase | Typical Use |
|-------------|------------|-------------|
| Tier 2 | +100% | Early game, testing |
| Tier 3 | +200% | Mid game, defensive ops |
| Tier 4 | +300% | End game, serious combat |

**Resource Cost Example (XUH2O):**
- 3000 U (utrium)
- 3000 H (hydrogen) 
- 3000 O (oxygen)
- 3000 X (catalyst)

**Total base minerals:** 12,000 for 3000 XUH2O

---

## Quick Reference Table

| Effect | Tier 2 | Tier 3 | Tier 4 (Best) |
|--------|--------|--------|---------------|
| Attack | UH | UH2O | **XUH2O** |
| Heal | UO | UHO2 | **XUHO2** |
| Ranged | KO | KHO2 | **XKHO2** |
| Build | LH | LH2O | **XLH2O** |
| Repair | LO | LHO2 | **XLHO2** |
| Dismantle | ZH | ZH2O | **XZH2O** |
| Move | ZO | ZHO2 | **XZHO2** |
| Upgrade | GH | GH2O | **XGH2O** |
| Tough | GO | GHO2 | **XGHO2** |
| Carry | KH | KH2O | **XKH2O** |
